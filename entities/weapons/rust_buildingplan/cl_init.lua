...
local function ClientModel(swep, status, pos, rot)
    if(!IsValid(swep.ghostEntity))then
        ...
        swep.ghostEntity.RenderOverride = function(swep) render.SuppressEngineLighting(true) swep:DrawModel() render.SuppressEngineLighting(false) end
    ...
    if swep:GetNetworkedString("selectedBuilding") == "foundation" then
        canPlace = canPlace and CheckGroundSupport(pos, "foundation")
    end
...
local function GetBuildingPositionAndAngle(buildingType, parentType, positionNumber)
    ...
    if buildingsTable[buildingType] and buildingsTable[buildingType]["parent"] and buildingsTable[buildingType]["parent"]["positions"] then
        ...
        if posData[parentType] and posData[parentType][positionNumber] then
            return posData[parentType][positionNumber]["position"], posData[parentType][positionNumber]["angle"]
        end
        
        -- РЎС‚Р°СЂР°СЏ СЃС‚СЂСѓРєС‚СѓСЂР° (РґР»СЏ СЃРѕРІРјРµСЃС‚РёРјРѕСЃС‚Рё)
        if posData[positionNumber] and posData[positionNumber]["position"] then
            return posData[positionNumber]["position"], posData[positionNumber]["angle"]
        end
    end
...
function SWEP:Think()
    ...
    if not self.LastPieMenuClose then
        self.LastPieMenuClose = 0
    end
    ...
    if(input.WasMousePressed( MOUSE_LEFT ) && canPlace && !self.PieOpened)then
