...
function ENT:Interact(pl)
    ...
    if dot < 0 then self:ConstructInventory(gRust.Inventory.RightPanel) end
end
...
function ENT:ConstructInventory(pnl, data, rows)
    if self:GetForward():Dot(self:GetPos() - LocalPlayer():GetPos()) > 0 then
        ...
        Container.AddSellOrder = function(me, item1, amount1, item2, amount2, index, instock)
            ...
            ButtonContainer.PerformLayout = function(me, w, h) BuyButton:SetTall(h * 0.5) end
        end
...
function ENT:AdminMenu(pl)
    ...
    AddButton.DoClick = function(me)
        ...
        if not self.ForItem then return end
    ...
    ItemList.SearchFor = function(me, str)
        ...
        for k, v in pairs(gRust.Items) do
            if string.find(string.lower(v:GetName()), str) then
                ...
                item.OnCursorEntered = function(me) me:ColorTo(Color(200, 200, 200, 200), 0.1) end
                item.OnCursorExited = function(me) me:ColorTo(Color(125, 125, 125, 200), 0.1) end
                item.DoClick = function()
                    if not self.LastType then return end
    ...
    for i = 1, 2 do
        ...
        Text.OnReleased = function(me) Frame:SetKeyboardInputEnabled(false) end
        Text.OnTextChanged = function(me, txt) ItemList:SearchFor(txt) end
        ...
        Dec.DoClick = function() self[Str .. "Amount"] = math.max(self[Str .. "Amount"] - 1, 1) end
        ...
        Input.Think = function(me)
            OThink(me)
            if tonumber(me:GetValue()) ~= self[Str .. "Amount"] then me.TextEntry:SetText(self[Str .. "Amount"]) end
        end

        Input.OnPressed = function(me) Frame:MakePopup() end
        Input.OnReleased = function(me) Frame:SetKeyboardInputEnabled(false) end
        Input.OnTextChanged = function(me, txt) self[Str .. "Amount"] = math.max(tonumber(txt) or 1, 1) end
        local Inc = Amount:Add("gRust.Button")
        Inc:Dock(RIGHT)
        Inc:SetIcon(gRust.GetIcon("add"))
        Inc.DoClick = function() self[Str .. "Amount"] = self[Str .. "Amount"] + 1 end
        Amount.PerformLayout = function(me, w, h) Dec:SetWide(h) end
    ...
    Items.PerformLayout = function(me, w, h)
        for k, v in ipairs(me:GetChildren()) do
            v:SetWide(w * 0.5)
            v:DockPadding(k == 1 and 0 or margin, 0, k == 1 and margin or 0, 0)
        end
    end
...
net.Receive("gRust.SendSellOrders", function()
    ...
    for i = 1, net.ReadUInt(12) do
        ...
        --     InStock = InStock
        -- }
        if VendingMachinePanel then VendingMachinePanel:AddSellOrder(Item1, Amount1, Item2, Amount2, i, InStock) end
    end
end)