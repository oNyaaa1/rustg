...
if SERVER then
    ...
    net.Receive("rust_minicopter", function(len)
        ...
        elseif process == 3 then
            ...
            if ent.SetHealth then ent:SetHealth(health) end
            ent.health = health
        elseif process == 4 then
            local ent = net.ReadEntity()
            local fuel = net.ReadUInt(9)
            if not IsValid(ent) then return end
            if ent.SetFuel then ent:SetFuel(fuel) end
            ...
            if LocalPlayer():GetPos():DistToSqr(ent:GetPos()) < 250000 then ent:ManipulateBoneAngles(1, Angle(0, 1 / 1171875000 * fuel ^ 4 - 3 / 7812500 * fuel ^ 3 - 101 / 375000 * fuel ^ 2 + 243 / 500 * fuel - 106, 1 / 585937500 * fuel ^ 4 - 2 / 1171875 * fuel ^ 3 - 1 / 37500 * fuel ^ 2 + 17 / 75 * fuel)) end
            ent.fuel = fuel
    ...
    function ENT:enableEffects()
        if self.engineActive then
            if not self.isActiveCL then
                ...
                if CurTime() - self.stopTime < 0.5 then self.startTime = self.startTime - 3 end
            end
...
function ENT:soundThink()
    ...
    if self.isActive then
        if self.rotorActive then
            ...
            if CurTime() > self.isActiveTime + 1.25 and (not self.rotorSound2:IsPlaying()) then self.rotorSound2:Play() end
    ...
    elseif not self.isActive then
        if self.rotorActive then
            if CurTime() > self.isStopTime + 0.75 then
                ...
                timer.Simple(2, function() if self.stopsound then self.stopsound:Stop() end end)
            end
            ...
            if self.startingSound:IsPlaying() then
                ...
                self.rotorSound2:Stop()
                self.stoppingSound:Play()
                timer.Simple(2, function() if self.stoppingSound then self.stoppingSound:Stop() end end)
            end
        end
    end
end
...
function ENT:Initialize()
    if SERVER then
        ...
        if IsValid(phys) then
            phys:SetMass(800)
            timer.Simple(0.15, function() if IsValid(phys) then phys:Wake() end end)
        end
...
if SERVER then
    ...
    function ENT:OnRemove()
        ...
        if self.rotorSound2 then self.rotorSound2:Stop() end
        if self.startingSound then self.startingSound:Stop() end
        if self.stopsound then self.stopsound:Stop() end
    end
    ...
    function ENT:ProcessDamage()
        if self.health <= 0 then
            timer.Simple(0, function() if IsValid(self) then self:StopMotionController() end end)
            if self.fuel > 0 then
                util.BlastDamage(self, self.driver or self or game.GetWorld(), self:GetPos(), 350, 100)
                util.ScreenShake(self:GetPos(), 100, 10, 1, 1000)
            end

            if IsValid(self.driver) and self.driver:IsPlayer() then self.driver:TakeDamage(1000, self, self) end
            if IsValid(self.gunner) and self.gunner:IsPlayer() then self.gunner:TakeDamage(1000, self, self) end
    ...
    function ENT:PhysicsSimulate(phys, delta)
        ...
        if ply and ply:IsValid() then
            if CurTime() > self.fuelTime then
                ...
                self:NetworkNotifyPVS(false)
                if self.fuel <= 0 then self:StopMotionController() end
            end
