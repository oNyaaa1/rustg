...
function ENT:Interact(pl)
	...
	if self.Authorized and not self.Authorized[pl:SteamID()] then return end
	if not self.Moving then
		...
		self.Moving = true
		timer.Simple(TimeToOpen, function() self.Moving = false end)
	end
...
function ENT:Think()
	if SERVER then
		...
		if not self.WasOpened or opened ~= self.WasOpened then self.PhysCollide = CreatePhysCollideBox(self.Mins, self.Maxs) end
