...
function PLAYER:GetNextCraftIndex()
    ...
    if self.CraftQueueIndex > (2^CRAFT_CONFIG.MAX_INDEX_BITS - 1) then
        self.CraftQueueIndex = 1
    end
...
function PLAYER:AddToCraftQueue(item, amount, skin)
    ...
    for _, recipe in ipairs(craftRecipe) do
        if not self:HasItem(recipe.item, recipe.amount * amount) then
            if self.SendNotification then
                self:SendNotification("Insufficient Resources", NOTIFICATION_REMOVE, "resources", "")
            end
            return false
        end
    end
    ...
    for _, recipe in ipairs(craftRecipe) do
        ...
        local icon = "materials/icons/close.png","smooth mips"
        
        if self.SendNotification then
            self:SendNotification(displayText, NOTIFICATION_REMOVE, icon, side)
        end
    end
...
function PLAYER:CancelCraft(index)
    if not self.CraftQueue or not self.CraftQueue[index] then 
        return false 
    end

    local craft = self.CraftQueue[index]
    if craft.completed then 
        return false 
    end
    ...
    for _, recipe in ipairs(craftRecipe) do
        ...
        if refundAmount > 0 then
            self:GiveItem(recipe.item, refundAmount)
        end
    end

    if self.RemoveCraftNotification then
        self:RemoveCraftNotification(index)
    end
