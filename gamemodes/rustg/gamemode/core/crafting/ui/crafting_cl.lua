...
local function LeftPanel()
	...
	for k, v in pairs(gRust.CraftQueue) do
		if v.item and v.time and gRust.Items[v.item] then
			QueueButtons[k] = AddQueueItem(gRust.Items[v.item], v.time, k)
		end
	end
...
local function RightPanel()
	...
	CraftPanel.Paint = PaintBackground
		...
		Dec.DoClick = function()
			...
			if gRust.Items[SelectedItem] then
		...
		Craft.DoClick = function(me)
			gRust.RequestCraft(SelectedItem, CraftAmount, SelectedSkin)
		end
	...
	MainPanel.Paint = PaintBackground
		...
		Image:Dock(LEFT)
		TopPanel.PerformLayout = function(me, w, h)
			Image:SetWide(h)
		end
	...
	gRust.CraftingMenu.SelectItem = function(item)
		...
		for k, v in ipairs(item:GetCraft()) do
			...
			for i, j in ipairs(CraftList:GetChildren()[k]:GetChildren()) do
				...
				if (!LocalPlayer():HasItem(v.item, v.amount)) then
					txt:SetColor(Color(255, 205, 98))
				end
	...
	timer.Simple(0.1, function()
		if IsValid(gRust.CraftingMenu) then
			for index, craft in pairs(gRust.CraftQueue) do
				if craft.item and craft.time and gRust.Items[craft.item] and not IsValid(QueueButtons[index]) then
					QueueButtons[index] = AddQueueItem(gRust.Items[craft.item], craft.time, index)
				end
			end
		end
	end)
