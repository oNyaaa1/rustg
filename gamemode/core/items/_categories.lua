...
function gRust.CreateCategory(name, icon)
    gRust.Categories[#gRust.Categories + 1] = {
        name = name,
        icon = icon
    }
    ...
    hook.Add("InitPostEntity", "gRust.LoadFavorites", function()
        ...
        if favorites then
            for _, v in ipairs(favorites) do
                if not gRust.Items[v.item] then continue end
...
function gRust.GetItemsByCategory(categoryName)
    local items = {}
    for itemClass, itemData in pairs(gRust.Items) do
        if itemData:GetCategory() == categoryName then table.insert(items, itemClass) end
    end
    return items
end
...
function gRust.GetItemRegister(itemClass)
    ...
    return {
        ...
        IsInCategory = function(self, category) return item:GetCategory() == category end,
