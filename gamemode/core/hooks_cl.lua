...
function Legs:SetUp()
    ...
    self.LegEnt.GetPlayerColor = function() return lp:GetPlayerColor() end
...
function Legs:WeaponChanged()
    if IsValid(self.LegEnt) then
        ...
        self.BonesToRemove = {"ValveBiped.Bip01_Head1", "ValveBiped.Bip01_L_Hand", "ValveBiped.Bip01_L_Forearm", "ValveBiped.Bip01_L_Upperarm", "ValveBiped.Bip01_L_Clavicle", "ValveBiped.Bip01_R_Hand", "ValveBiped.Bip01_R_Forearm", "ValveBiped.Bip01_R_Upperarm", "ValveBiped.Bip01_R_Clavicle", "ValveBiped.Bip01_Spine4", "ValveBiped.Bip01_Spine2"}
...
function ShouldDrawLegs()
    return LegsBool:GetBool() and IsValid(Legs.LegEnt) and lp:Alive() and GetViewEntity() == lp and not lp:ShouldDrawLocalPlayer() and not holdingbind(lp) and not lp:InVehicle()
end
...
function Legs:DoFinalRender()
    ...
    if ShouldDrawLegs() then
        ...
        if lp:GetGroundEntity() == NULL then
            ...
            if lp:KeyDown(IN_DUCK) then self.RenderPos.z = self.RenderPos.z - 28 end
        end
...
hook.Add("UpdateAnimation", "LegsUpdateAnimation", function(ply, velocity, maxseqgroundspeed)
    if ply == lp then
        if IsValid(Legs.LegEnt) then
            ...
            if string.lower(lp:GetModel()) ~= string.lower(Legs.LegEnt:GetModel()) then Legs:SetUp() end
        else
            Legs:SetUp()
        end
    end
...
hook.Add("PostDrawTranslucentRenderables", "LegsRender", function() if lp then Legs:DoFinalRender() end end)
timer.Simple(3, function() if IsValid(lp) then Legs:SetUp() end end)
hook.Add("PlayerSpawn", "LegsSpawn", function(ply) if ply == lp then timer.Simple(1, function() Legs:SetUp() end) end end)