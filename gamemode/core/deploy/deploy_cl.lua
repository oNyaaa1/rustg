...
function gRust.RequestDeploy(index)
    ...
    if not IsValid(gRust.DeployEntity) then
        ...
        if DeployData.ModelScale then gRust.DeployEntity:SetModelScale(DeployData.ModelScale) end
    end
...
hook.Add("Think", "gRust.DeployRotation", function()
    ...
    if r and not lastR then DeployRotation = (DeployRotation + 90) % 360 end
    lastR = r
end)
...
local function CheckPlace(pl, key)
    ...
    net.SendToServer()
    if pl.Inventory[DeployIndex]:GetQuantity() <= 1 then gRust.ResetDeploy() end
end
