...
local function HandleDeploy(_, pl)
    ...
    if Deploy.Socket == "lock" then
        ...
        if not IsValid(tr.Entity) or not string.find(tr.Entity:GetClass(), "door") then 
            return 
        end
        
        if tr.Entity:GetBodygroup(2) ~= 0 then 
            return 
        end

        if tr.Entity:GetNW2Bool("gRust.LockInUse", false) then
            return
        end
    ...
    elseif Deploy.Socket then
        ...
        if Deploy.Socket == "lock" and IsValid(hitEnt) then
            if hitEnt.GetBodygroup and hitEnt:GetBodygroup(2) ~= 0 then
                return
            end
            if hitEnt:GetNW2Bool("gRust.LockInUse", false) then
                return
            end
        end
    ...
    if Deploy.Socket == "lock" and IsValid(hitEnt) then
        hitEnt:SetNW2Bool("gRust.LockInUse", true)
    end
