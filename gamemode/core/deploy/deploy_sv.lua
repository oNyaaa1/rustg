...
local function HandleDeploy(len, pl)
    ...
    if (!DeployData) then return end
    
    -- Check if player can deploy at this position
    if (!pl:CanDeploy(DeployData)) then return end
    ...
    if (!pl:HasBuildPrivilege(pos, gRust.Config.TCRadius^2 * 4)) then return end
    
    -- Check for entity collision if not using sockets
    if (!DeployData.Socket) then
    ...
    if (ent.SetNW2String) then
        ent:SetNW2String("gRust.OwnerSteamID", pl:SteamID())
    end
    ...
    if (DeployData.Socket and IsValid(hitEnt) and hitEnt:GetClass() == "rust_structure") then
        ...
        if (ent.SetNW2Bool) then
            ent:SetNW2Bool("gRust.InUse", true)
        end
    end
    
    -- Custom spawn logic
    if (DeployData.OnSpawn) then
        DeployData.OnSpawn(ent, pl)
    end
    ...
    if (Item:GetQuantity() <= 0) then
        ...
        if (pl.SelectedSlotIndex == index) then
            ...
            if (IsValid(activeWeapon)) then
                pl:StripWeapon(activeWeapon:GetClass())
            end
        end
    end
    
    if ( pl:SyncInventory()) then
        pl:SyncInventory()
    end
...
function PLAYER:ServerCanDeploy(pos, data)
    ...
    if (!self:HasBuildPrivilege(pos, gRust.Config.TCRadius^2 * 4)) then 
    ...
    if (data.RequiresFoundation) then
        ...
        if (!foundationFound) then
            return false, "Requires foundation"
        end
    end
    ...
    if (data.MaxHeight) then
        ...
        if (!trace.Hit) then
            return false, "Too high from ground"
        end
    end
    
    -- Check water deployment
    if (data.NoWater and util.PointContents(pos) == CONTENTS_WATER) then
        return false, "Cannot deploy in water"
    end
...
hook.Add("PlayerUse", "gRust.DeployPickup", function(pl, ent)
    ...
    if (!IsValid(owner) and ownerSteamID != "") then
    ...
    if (!entTable or !entTable.Deploy) then return end
    
    local deployData = entTable.Deploy
    if (!deployData.CanPickup) then return end
    
    -- Check if player is holding proper tool (hammer, etc.)
    local weapon = pl:GetActiveWeapon()
    if (!IsValid(weapon) or weapon:GetClass() ~= "rust_hammer") then return end
    
    -- Add item back to inventory
    local itemClass = deployData.Item or ent:GetClass():gsub("rust_", "")
    if (pl.AddItem) then
        pl:AddItem(itemClass, 1)
    end
    
    -- Handle socket cleanup
    if (deployData.Socket and ent:GetParent()) then
        ent:SetNW2Bool("gRust.InUse", false)
    end
    
    -- Custom pickup logic
    if (deployData.OnPickup) then
        deployData.OnPickup(ent, pl)
    end
