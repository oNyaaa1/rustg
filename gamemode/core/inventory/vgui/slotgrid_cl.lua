...
function PANEL:PerformLayout()
    if not self.NeedLayout then return end
    ...
    for x = 1, Rows do
        ...
        Row:Dock(TOP)
        if x < Rows then Row:DockMargin(0, 0, 0, Margin) end
        for y = 1, Cols do
            ...
            Slot.OnQuickSwap = function(me)
                if Ent == LocalPlayer() then
                    ...
                    if not Slot or not me.Item then return end
                    LocalPlayer():MoveSlot(Ent, LocalPlayer(), me:GetID(), Slot, me.Item:GetQuantity())
                end
            end

            Slot.DoClick = function(me)
                if IsValid(LocalPlayer().SelectedSlotPanel) then LocalPlayer().SelectedSlotPanel:SetSelected(false) end
            ...
            if y < Cols then Slot:DockMargin(0, 0, Margin, 0) end
            self.Slots[i] = Slot
            i = i + 1
        end
    end
