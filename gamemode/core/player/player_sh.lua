function GM:Move(pl, mv)
	...
	if mv:KeyPressed(IN_JUMP) and pl:IsOnGround() then
...
function gRust.FindPlayer(id, requester)
	...
	if id == "^" then return requester end
	if id == "@" and IsValid(requester) and requester:IsPlayer() then return requester:GetEyeTrace().Entity end
	id = string.lower(id)
	local Players = player.GetAll()
	for i = 1, #Players do
		local pl = Players[i]
		if string.find(string.lower(pl:Name()), id) or pl:SteamID64() == id then return pl end
	end
end

PLAYER.gRust = true
timer.Simple(0, function() PLAYER.DisplayIcon = gRust.GetIcon("add") end)
...
function GM:ShouldCollide(ent1, ent2)
	...
	if ent and ent == ent2 or ent == "*" then return false end
	return true
end