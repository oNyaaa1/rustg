...
function PANEL:Init()
	...
	Scrap.PerformLayout = function() Scrap:SetWide(scrh * 0.125) end
	...
	Scrap.Paint = function(me, w, h)
		if OldScrapPaint then OldScrapPaint(me, w, h) end
	...
	Unlock.DoClick = function(me)
		if not me.Path then return end
	...
	Close.Paint = function(me, w, h)
		...
		if me:IsHovered() then col = color_white end
		draw.SimpleText("CLOSE", "gRust.52px", w * 0.5, h * 0.5, col, 1, 1)
	end

	Close.DoClick = function(me) self:Remove() end
	...
	ItemIcon:Dock(LEFT)
	ItemIcon.PerformLayout = function(me) me:SetWide(me:GetTall()) end
	ItemIcon.Paint = function(me, w, h)
		if not self.SelectedItem then return end
		surface.SetDrawColor(255, 255, 255)
		surface.SetMaterial(Material(gRust.Items[self.SelectedItem]:GetIcon(), "smooth mips"))
...
function PANEL:InsertTree(tree, parent, row, locked, dir)
	...
	for k, v in ipairs(tree) do
		...
		Item:SetLocked(locked and not LocalPlayer():HasBlueprint(v.Item))
		...
		Item:SetItem(v.Item)
		if parent then Item:Connect(parent) end
		...
		if v.Branch then self:InsertTree(v.Branch, v.Item, row, locked or not LocalPlayer():HasBlueprint(v.Item), v.Direction or dir) end
	end
